...
class WhenDidSicknessBeginFormProviderSpec extends DateBehaviours {
  ...
  ".value" - {
    ...
    behave like dateFieldWithMin(
      ...
      key = "value",
      min = minDate,
      formError = FormError("value", "whatIsYourDOB.error.beforeMin", Seq(maxDate.format(dateFormatter)))
    )
    ...
    behave like dateFieldWithMax(
      ...
      key = "value",
      max = maxDate,
      formError = FormError("value", "whatIsYourDOB.error.afterMax", Seq(maxDate.format(dateFormatter)))
    )
  }
}
